...
#include <amxmisc>
#include <cstrike>
#include <survival_mixsystem_pts>
...
new Trie:g_tIpPrefixes, g_iTrieIpSize;
...
LoadPlayersPrefixes()
{
    ...
    new dateTime[32]; get_time("%d:%m:%Y %H:%M:%S:", dateTime, 31);
    ...
    g_tIpPrefixes = TrieCreate();
    g_aFlagPrefixes = ArrayCreate(FLAG_PREFIX_INFO);
    
    new file = fopen(file_name, "rt");

    if(file) {
        new text[256], dataId[32], endTime[32], type[10], auth[32], prefix[PREFIX_MAX_LENGTH + COLOR_BUFFER], prefix_info[FLAG_PREFIX_INFO];
        while(!feof(file)) {
            fgets(file, text, charsmax(text));

            parse(text, dataId, charsmax(dataId), endTime, charsmax(endTime), type, charsmax(type), auth, charsmax(auth), prefix, charsmax(prefix));
            
            new curDay, curMonth, curYear, curHour, curMinute, curSecond;
            new fileDay, fileMonth, fileYear, fileHour, fileMinute, fileSecond;

            if (!custom_parse_time(dateTime, curDay, curMonth, curYear, curHour, curMinute, curSecond)) {
                log_amx("Failed to parse current time");
                continue;
            }
            if (!custom_parse_time(endTime, fileDay, fileMonth, fileYear, fileHour, fileMinute, fileSecond)) {
                log_amx("Failed to parse file time");
                continue;
            }
            if (compare_times(curYear, curMonth, curDay, curHour, curMinute, curSecond, fileYear, fileMonth, fileDay, fileHour, fileMinute, fileSecond))
                continue;

            if(!dataId[0] || dataId[0] == ';' || !endTime[0] || !type[0]  || !auth[0] || !prefix[0]) continue;
            ...
            switch(type[0]) {
                ...
                case 'i':
				{
					TrieSetString(g_tIpPrefixes, auth, prefix);
					g_iTrieIpSize++;
				}
                //flag
                case 'f': {
...
public client_putinserver(id)
{
    ...
    new sIP[16]; get_user_ip(id, sIP, charsmax(sIP), 1);
    if(g_iTrieIpSize && TrieKeyExists(g_tIpPrefixes, sIP))
    {
        g_bCustomPrefix[id] = true;
        TrieGetString(g_tIpPrefixes, sIP, g_sPlayerPrefix[id], charsmax(g_sPlayerPrefix[]));
    }
    #endif
...
public FormatMessage(sender, CsTeams:sender_team, channel, name_color, chat_color, time_code[], name[], message[])
{
    ...
    #if defined FUNCTION_PTS_PREFIX
    len += formatex(text[len], charsmax(text) - len, "^1[^3%s^1] ", g_eSkillData[survival_pts_get_player_info(sender, skill)][SKILL_LVL]);
    #endif

    #if defined FUNCTION_PLAYER_PREFIX
    if (!equali(g_sPlayerPrefix[sender], ""))
...
stock bool:custom_parse_time(const timeString[], &day, &month, &year, &hour, &minute, &second) {
    new pos = 0;
    pos = parse(timeString, pos, day);
    if (pos < 0) return false;
    pos = parse(timeString, pos + 1, month);
    if (pos < 0) return false;
    pos = parse(timeString, pos + 1, year);
    if (pos < 0) return false;
    pos = parse(timeString, pos + 1, hour);
    if (pos < 0) return false;
    pos = parse(timeString, pos + 1, minute);
    if (pos < 0) return false;
    pos = parse(timeString, pos + 1, second);
    return pos >= 0;
}
stock bool:compare_times(
    curYear, curMonth, curDay, curHour, curMinute, curSecond,
    fileYear, fileMonth, fileDay, fileHour, fileMinute, fileSecond
) {
    if (curYear > fileYear) return true;
    if (curYear < fileYear) return false;

    if (curMonth > fileMonth) return true;
    if (curMonth < fileMonth) return false;

    if (curDay > fileDay) return true;
    if (curDay < fileDay) return false;

    if (curHour > fileHour) return true;
    if (curHour < fileHour) return false;

    if (curMinute > fileMinute) return true;
    if (curMinute < fileMinute) return false;

    if (curSecond > fileSecond) return true;
    return false;
}
